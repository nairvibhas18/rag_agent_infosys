...
async def ingest_pdf_to_chromadb(
    ...
    chunks = split_text_into_chunks(text, max_words=max_words, overlap=overlap)
...
async def ingest_pdf_file_to_chromadb(
    pdf_bytes: bytes,
    agent_deps,
    metadata: Dict[str, Any] = None,
    max_words: int = 400,
    ...
    chunks = split_text_into_chunks(text, max_words=max_words, overlap=overlap)
    ids = [f"uploaded__{i}" for i in range(len(chunks))]
    metadatas = [metadata or {"source": "uploaded"}] * len(chunks)
    embeddings = agent_deps.embedding_func(chunks)
    collection = get_or_create_collection(
        agent_deps.chroma_client,
        agent_deps.collection_name,
        embedding_model_name=agent_deps.embedding_model
    )
    collection.add(
        ids=ids,
        documents=chunks,
    ...
    return f"PDF ingested successfully. {len(chunks)} chunks added."
